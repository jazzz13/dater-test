...
import type { ActionRequestType, GameType } from '../types';

const functions = require('firebase-functions/lib/index');
const admin = require('firebase-admin/lib/index');
const { GAMES_COLLECTION, ROUNDS_COUNT, FINISHED_STATE } = require('../const');
const { httpMethodsWrap, checkTypeParam } = require('../helpers');
const { nextPlayerName, calculateScores, selectWinners } = require('./helpers');

module.exports = functions.https.onRequest(httpMethodsWrap(async (req, res) => {
  const {
    player, gameId, rivalDigit, myDigit,
  } = (req.body.data: ActionRequestType);
